...
if ( $nv_Request->isset_request( 'submit', 'post' ) )
{
	...
	if( empty( $error ) )
	{
		try
		{
			...
			{
				$stmt = $db->prepare( 'UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_cat SET title = :title, alias = :alias, description = :description, keywords = :keywords, image = :image WHERE id=' . $row['id'] );
			}
